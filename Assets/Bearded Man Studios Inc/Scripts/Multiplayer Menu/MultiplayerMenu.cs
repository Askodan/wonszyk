...
public class MultiplayerMenu : MonoBehaviour
{
    ...
    {
        StartCoroutine(LookForServers());
    }

    System.Collections.IEnumerator LookForServers()
    {
        while (true)
        {
    ...
    {
        //endpoint.port jakby kto potrzebował
        foundIp.text = endpoint.Address;
    }
    public void PasteAdress()
    {
    ...
    public void Host()
    {
        ...
        else
        {
            ...
            if (natServerHost.Trim().Length == 0)
                ((UDPServer)server).Connect("0.0.0.0", ushort.Parse(portNumber.text));
            else
