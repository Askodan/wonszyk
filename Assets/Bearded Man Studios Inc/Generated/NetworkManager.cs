...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] WonszykLogicNetworkObject = null;
		public GameObject[] WonszykPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is WonszykLogicNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (WonszykLogicNetworkObject.Length > 0 && WonszykLogicNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(WonszykLogicNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<WonszykLogicBehavior>();
						}
					}
			...
			else if (obj is WonszykPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (WonszykPlayerNetworkObject.Length > 0 && WonszykPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(WonszykPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<WonszykPlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateWonszykLogic instead, its shorter and easier to type out ;)")]
		public WonszykLogicBehavior InstantiateWonszykLogicNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(WonszykLogicNetworkObject[index]);
			var netBehavior = go.GetComponent<WonszykLogicBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<WonszykLogicBehavior>().networkObject = (WonszykLogicNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateWonszykPlayer instead, its shorter and easier to type out ;)")]
		public WonszykPlayerBehavior InstantiateWonszykPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(WonszykPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<WonszykPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<WonszykPlayerBehavior>().networkObject = (WonszykPlayerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of WonszykLogic
		/// </summary>
		/// <returns>
		/// A local instance of WonszykLogicBehavior
		/// </returns>
		/// <param name="index">The index of the WonszykLogic prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public WonszykLogicBehavior InstantiateWonszykLogic(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(WonszykLogicNetworkObject[index]);
			var netBehavior = go.GetComponent<WonszykLogicBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of WonszykPlayer
		/// </summary>
		/// <returns>
		/// A local instance of WonszykPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the WonszykPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public WonszykPlayerBehavior InstantiateWonszykPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(WonszykPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<WonszykPlayerBehavior>();
